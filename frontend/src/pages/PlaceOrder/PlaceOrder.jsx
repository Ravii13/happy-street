...
const PlaceOrder = () => {
  ...
  // Destructure setCartItems as well, to clear the cart after successful payment
  ...
  const placeOrder = async (event) => {
    ...
    food_list.map((item) => {
      if (cartItems[item._id] > 0) {
        let itemInfo = { ...item } // Create a copy to avoid mutating original item
    ...
    if (response.data.success) {
      ...
      // clear cart and redirect to home after user closes the alert
      try {
        setCartItems({})
      } catch (e) {}
      navigate('/')
